...
const Navbar = React.memo((props: AppProps) => {
	const handleQuery = (queryName: string) => {
		props.setQuery(queryName);
		props.setValue(`select * from ${queryName}`);
	};
	return (
		<>
			<nav className={`${props.isOpen ? 'col-start-2' : 'col-start-1'} col-end-3 row-start-1 row-end-2 bg-primary-dark shadow`}>
					...
					<div className="flex items-center justify-between h-16">
						<div className="w-full flex justify-between items-center">
							...
							<a
								...
								href="https://github.com/prashantkr006/Atlan-frontend-SQL-editor"
								target="_blank"
								rel="noreferrer"
								title="github repository"
							>
								<svg width="24" height="24" fill="currentColor" className="text-lg leading-lg text-white opacity-75 hover:opacity-100 transition-opacity">
				...
				<div className="relative col-start-1 col-end-2 row-start-1 row-end-5">
					<div className="bg-secondary-dark text-white w-72 px-3 fixed top-0 bottom-0 z-50 min-h-screen overflow-y-auto">
						<svg
							...
							fill="currentColor"
							onClick={() => {
								return props.setIsOpen(!props.isOpen);
							}}
						>
